...
private:
  ...
  ResourcesContainer *resContainter =
      new ResourcesContainer; ///< resorces container (include all textures and
                              ///< fonts)
  ...
  std::vector<Bullet *>::const_iterator
      bullet_iter;                    ///< iterator for bullets vector
  std::vector<Bullet *> bulletsArray; ///< vector contains all bullet objects

  std::vector<Enemy *>::const_iterator
      enemy_iter;                    ///< iterator for enemies vector
  std::vector<Enemy *> enemiesArray; ///< vector contains all enemies objects

  std::vector<textDisplay *>::const_iterator
      text_iter;                         ///< iterator for text vector
  std::vector<textDisplay *> textsArray; ///< vector contains all texts

  std::vector<Item *>::const_iterator
      item_iter;                  ///< iterator for weapon item vector
  std::vector<Item *> itemsArray; ///< vector contains all item objects

  std::vector<Weapon *>::const_iterator
      weapon_iter;                   ///< iterator for weapon vector
  ...
  sf::Clock player_enemy_collision_clocker; ///< clocker for updating player and
                                            ///< enemies collisions
  sf::Time bullets_timer;                   ///< timer for updating player atack
  sf::Time player_enemy_collision_timer;    ///< timer for updating player and
                                            ///< enemies collisions
  ...
  // Update functions
  /**
   * @brief Method updateing all sfml events (such as button pressing, closing
   * window, etc)
   */
