...
namespace {
    ...
    void updateIdentification(unsigned int index) {
        ...
        texts["ID"].value.setString(
                sf::Joystick::getIdentification(index).name);
    }

    // Update joystick axes
    void updateAxes(unsigned int index) {
        for (unsigned int j = 0; j < sf::Joystick::AxisCount; ++j) {
            if (sf::Joystick::hasAxis(index,
                                      static_cast<sf::Joystick::Axis>(j)))
                set(axislabels[j], sf::Joystick::getAxisPosition(index,
                                                                 static_cast<sf::Joystick::Axis>(j)));
        }
    }
...
int main() {
    ...
    for (unsigned int i = 0; i < sf::Joystick::ButtonCount; ++i) {
        ...
        object.value.setPosition(80.f, 5.f +
                                       ((sf::Joystick::AxisCount + i + 4) *
                                        font.getLineSpacing(14)));
        object.value.setString("N/A");
    }
    ...
    while (window.isOpen()) {
        ...
        while (window.pollEvent(event)) {
            ...
            if ((event.type == sf::Event::Closed) ||
                ...
                for (Texts::iterator it = texts.begin();
                     it != texts.end(); ++it)
        ...
        for (Texts::const_iterator it = texts.begin();
             it != texts.end(); ++it) {
