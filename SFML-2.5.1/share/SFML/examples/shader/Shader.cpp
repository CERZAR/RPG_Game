...
public:
  ...
  bool onLoad() {
    ...
    m_text.setString(
        ...
        "aliquam.\n"
        "Mauris mi odio, bibendum quis fringilla a, laoreet vel orci. Proin "
        "vitae vulputate tortor.\n"
        ...
        "In hac habitasse platea dictumst. Etiam fringilla est id odio dapibus "
        "sit amet semper dui laoreet.\n");
...
public:
  ...
  void onUpdate(float time, float x, float y) {
    ...
    for (std::size_t i = 0; i < m_entities.size(); ++i) {
      ...
      position.x =
          std::cos(0.25f * (time * i + (m_entities.size() - i))) * 300 + 350;
      position.y =
          std::sin(0.25f * (time * (m_entities.size() - i) + i)) * 200 + 250;
