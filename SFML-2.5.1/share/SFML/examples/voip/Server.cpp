...
private:
  ...
  void receiveLoop() {
    while (!m_hasFinished) {
      ...
      if (id == audioData) {
        // Extract audio samples from the packet, and append it to our samples
        // buffer
        ...
        // Don't forget that the other thread can access the sample array at any
        // time (so we protect any operation on it with the mutex)
        {
          sf::Lock lock(m_mutex);
