...
int main() {
  ...
  bool prerequisitesSupported =
      sf::VertexBuffer::isAvailable() && sf::Shader::isAvailable();
  ...
  if (!prerequisitesSupported) {
  ...
  } else if (!terrainShader.loadFromFile("resources/terrain.vert",
    ...
    // Create our VertexBuffer with enough space to hold all the terrain
    // geometry
  ...
  statusText.setPosition(
      (windowWidth - statusText.getLocalBounds().width) / 2.f,
    ...
    if (prerequisitesSupported) {
      {
        ...
        // Don't bother updating/drawing the VertexBuffer while terrain is being
        // regenerated
        if (!pendingWorkCount) {
          // If there is new data pending to be uploaded to the VertexBuffer, do
          // it now
      ...
      osstr << "Frame:  " << clock.restart().asMilliseconds() << "ms\n"
            ...
            << "Use the arrow keys to change the values.\nUse the return key "
               "to regenerate the terrain.\n\n";

      for (int i = 0; i < settingCount; ++i)
        osstr << ((i == currentSetting) ? ">>  " : "       ")
...
float getElevation(float x, float y) {
  ...
  for (int i = 0; i < perlinOctaves; i++) {
    elevation += stb_perlin_noise3(
                     ...
                     y * perlinFrequency * std::pow(perlinFrequencyBase, i), 0,
                     0, 0, 0) *
  ...
  elevation = (elevation + heightBase) *
              (1.0f - edgeFactor * std::pow(distance, edgeDropoffExponent));
...
float getMoisture(float x, float y) {
  ...
  float moisture =
      stb_perlin_noise3(x * 4.f + 0.5f, y * 4.f + 0.5f, 0, 0, 0, 0);
...
sf::Color getLowlandsTerrainColor(float moisture) {
  sf::Color color =
      moisture < 0.27f
          ? sf::Color(240, 240, 180)
          : moisture < 0.3f
                ? sf::Color(240 - 240 * (moisture - 0.27f) / 0.03f,
                            240 - 40 * (moisture - 0.27f) / 0.03f,
                            180 - 180 * (moisture - 0.27f) / 0.03f)
                : moisture < 0.4f
                      ? sf::Color(0, 200, 0)
                      : moisture < 0.48f
                            ? sf::Color(0, 200 - 40 * (moisture - 0.4f) / 0.08f,
                                        0)
                            : moisture < 0.6f
                                  ? sf::Color(0, 160, 0)
                                  : moisture < 0.7f
...
sf::Color getHighlandsTerrainColor(float elevation, float moisture) {
  ...
  sf::Color color = moisture < 0.6f
                        ...
                        : sf::Color(112 + 110 * (moisture - 0.6f) / 0.4f,
...
sf::Color getTerrainColor(float elevation, float moisture) {
  sf::Color color =
      elevation < 0.11f
          ? sf::Color(0, 0, elevation / 0.11f * 74.f + 181.0f)
          : elevation < 0.14f
                ? sf::Color(std::pow((elevation - 0.11f) / 0.03f, 0.3f) * 48.f,
                            std::pow((elevation - 0.11f) / 0.03f, 0.3f) * 48.f,
                            255)
                : elevation < 0.16f
                      ? sf::Color((elevation - 0.14f) * 128.f / 0.02f + 48.f,
                                  (elevation - 0.14f) * 128.f / 0.02f + 48.f,
                                  127.0f + (0.16f - elevation) * 128.f / 0.02f)
                      : elevation < 0.17f
                            ? sf::Color(240, 230, 140)
                            : elevation < 0.4f
                                  ? getLowlandsTerrainColor(moisture)
                                  : elevation < snowcapHeight
                                        ? getHighlandsTerrainColor(elevation,
                                                                   moisture)
                                        : getSnowcapTerrainColor(elevation,
                                                                 moisture);
...
///
////////////////////////////////////////////////////////////
sf::Vector2f computeNormal(int x, int y, float left, float right, float bottom,
                           float top) {
  sf::Vector3f deltaX(
      1, 0,
      (std::pow(right, heightFlatten) - std::pow(left, heightFlatten)) *
          heightFactor);
  sf::Vector3f deltaY(
      0, 1,
      (std::pow(top, heightFlatten) - std::pow(bottom, heightFlatten)) *
          heightFactor);
...
///
////////////////////////////////////////////////////////////
void processWorkItem(std::vector<sf::Vertex> &vertices,
  ...
  const float scalingFactorX =
      static_cast<float>(windowWidth) / static_cast<float>(resolutionX);
  const float scalingFactorY =
      static_cast<float>(windowHeight) / static_cast<float>(resolutionY);
  ...
  for (unsigned int y = rowStart; y < rowEnd; y++) {
    for (int x = 0; x < resolutionX; x++) {
      ...
      if (x > 0) {
        ...
        vertices[arrayIndexBase + 0] =
            vertices[arrayIndexBase - resolutionX * 6 + 1];
      } else {
        vertices[arrayIndexBase + 0].position =
            sf::Vector2f(x * scalingFactorX, y * scalingFactorY);
        vertices[arrayIndexBase + 0].color =
            getTerrainColor(getElevation(x, y), getMoisture(x, y));
        vertices[arrayIndexBase + 0].texCoords =
            computeNormal(x, y, getElevation(x - 1, y), getElevation(x + 1, y),
                          getElevation(x, y + 1), getElevation(x, y - 1));
      }
      ...
      if (x > 0) {
        ...
        vertices[arrayIndexBase + 1].position =
            sf::Vector2f(x * scalingFactorX, (y + 1) * scalingFactorY);
        vertices[arrayIndexBase + 1].color =
            getTerrainColor(getElevation(x, y + 1), getMoisture(x, y + 1));
        vertices[arrayIndexBase + 1].texCoords = computeNormal(
            x, y + 1, getElevation(x - 1, y + 1), getElevation(x + 1, y + 1),
            getElevation(x, y + 2), getElevation(x, y));
      }

      // Bottom right corner (first triangle)
      vertices[arrayIndexBase + 2].position =
          sf::Vector2f((x + 1) * scalingFactorX, (y + 1) * scalingFactorY);
      vertices[arrayIndexBase + 2].color = getTerrainColor(
          getElevation(x + 1, y + 1), getMoisture(x + 1, y + 1));
      vertices[arrayIndexBase + 2].texCoords = computeNormal(
          x + 1, y + 1, getElevation(x, y + 1), getElevation(x + 2, y + 1),
          getElevation(x + 1, y + 2), getElevation(x + 1, y));
      ...
      if (y > rowStart) {
        vertices[arrayIndexBase + 5] =
            vertices[arrayIndexBase - resolutionX * 6 + 2];
      } else {
        vertices[arrayIndexBase + 5].position =
            sf::Vector2f((x + 1) * scalingFactorX, y * scalingFactorY);
        vertices[arrayIndexBase + 5].color =
            getTerrainColor(getElevation(x + 1, y), getMoisture(x + 1, y));
        vertices[arrayIndexBase + 5].texCoords = computeNormal(
            x + 1, y, getElevation(x, y), getElevation(x + 2, y),
            getElevation(x + 1, y + 1), getElevation(x + 1, y - 1));
      }
    }
  }

  // Copy the resulting geometry from our thread-local buffer into the target
  // buffer
  std::memcpy(workItem.targetBuffer + (resolutionX * rowStart * 6),
              &vertices[0], sizeof(sf::Vertex) * resolutionX * rowCount * 6);
}
