    ...
    for (int i = 1; i < 61; i++) {
        ...
        while (true) {
            enemy->rect.setPosition(
                    GenerateRandomFloat(static_cast<float>(map.startX),
                                        static_cast<float>(map.endX)),
                    GenerateRandomFloat(static_cast<float>(map.startY),
                                        static_cast<float>(map.endY)));
...
void Game::initWindow() {
    ...
    this->window = new sf::RenderWindow(sf::VideoMode(1360, 768), "Game",
                                        sf::Style::Close);
    window->setFramerateLimit(120);

    playerView.setSize(sf::Vector2f(static_cast<float>(window->getSize().x),
                                    static_cast<float>(window->getSize().y)));
    playerView.setCenter(
            sf::Vector2f(static_cast<float>(window->getSize().x / 2),
...
void Game::initPauseState() {
    pause_rect.setSize(sf::Vector2f(static_cast<float>(window->getSize().x),
                                    static_cast<float>(window->getSize().y)));
...
Game::~Game() {
    counter = 0;
    for (bullet_iter = bulletsArray.begin();
         bullet_iter != bulletsArray.end(); bullet_iter++) {
    ...
    counter = 0;
    for (text_iter = textsArray.begin();
         text_iter != textsArray.end(); text_iter++) {
        delete textsArray[counter];
        counter++;
    }

    counter = 0;
    for (item_iter = itemsArray.begin();
         item_iter != itemsArray.end(); item_iter++) {
        delete itemsArray[counter];
        counter++;
    }

    counter = 0;
    for (weapon_iter = weaponArray.begin();
         weapon_iter != weaponArray.end(); weapon_iter++) {
...
void Game::updateSFMLEvents() {
    while (this->window->pollEvent(this->event)) {
        ...
        if (this->event.type == sf::Event::KeyPressed &&
            this->event.key.code == sf::Keyboard::P) {
            if (gameState == PAUSE) gameState = GAME;
            else gameState = PAUSE;
        }
        if (this->event.type == sf::Event::KeyPressed &&
            this->event.key.code == sf::Keyboard::B) {
...
void Game::updateGUI() {
    hp_text.setString("HP: " + std::to_string(player->hp) + "/" +
                      std::to_string(player->maxHP));
    hp_text.setPosition(player->rect.getPosition().x - window->getSize().x / 2 +
                        window->getSize().x / 36,
                        player->rect.getPosition().y - window->getSize().y / 2 +
                        21);
    mp_text.setString("MP: " + std::to_string(player->mp) + "/" +
                      std::to_string(player->maxMP));
    mp_text.setPosition(player->rect.getPosition().x - window->getSize().x / 2 +
                        window->getSize().x / 36,
                        player->rect.getPosition().y - window->getSize().y / 2 +
                        41);
    if (sf::Keyboard::isKeyPressed(sf::Keyboard::Tab)) {
        gold_text.setString("Gold: " + std::to_string(player->coins));
        gold_text.setPosition(
                player->rect.getPosition().x - window->getSize().x / 2 +
                window->getSize().x / 36,
                player->rect.getPosition().y - window->getSize().y / 2 + 61);
        lvl_text.setString("Level: " + std::to_string(player->level));
        lvl_text.setPosition(
                player->rect.getPosition().x - window->getSize().x / 2 +
                window->getSize().x / 36,
                player->rect.getPosition().y - window->getSize().y / 2 + 81);
        exp_text.setString("Exp: " + std::to_string(player->exp) + "/" +
                           std::to_string(player->needExp));
        exp_text.setPosition(
                player->rect.getPosition().x - window->getSize().x / 2 +
                window->getSize().x / 36,
                player->rect.getPosition().y - window->getSize().y / 2 + 101);
    }
}
...
void Game::updatePressings() {
    ...
    if (bullets_timer.asMilliseconds() >= player->currentWeapon->atackSpeed) {
        if (sf::Keyboard::isKeyPressed(sf::Keyboard::Space) &&
            player->direction >= 0 && player->mp > 0) {
            player->mp--;
            if (player->currentWeapon->level == 5) {
                Bullet *bullet = player->currentWeapon->getBullet(UP);
                bullet->rect.setPosition(player->rect.getPosition().x +
                                         player->rect.getSize().x / 2 -
                                         bullet->rect.getSize().x / 2,
                                         player->rect.getPosition().y +
                                         player->rect.getSize().y / 2 -
                                         bullet->rect.getSize().y / 2);
                ...
                Bullet *bullet = player->currentWeapon->getBullet(
                        player->direction);
                bullet->rect.setPosition(player->rect.getPosition().x +
                                         player->rect.getSize().x / 2 -
                                         bullet->rect.getSize().x / 2,
    ...
    if (bullets_timer.asMilliseconds() >= 1) {
        if (sf::Keyboard::isKeyPressed(sf::Keyboard::E)) {
            ...
            enemy->rect.setPosition(
                    static_cast<float>(GenerateRandomInt(window->getSize().x)),
                    static_cast<float>(GenerateRandomInt(window->getSize().y)));
            enemiesArray.push_back(enemy);
            bullets_clocker.restart();
            counter++;
        }
...
void Game::updateCollisions() {
    ...
    for (bullet_iter = bulletsArray.begin();
        ...
        for (enemy_iter = enemiesArray.begin();
             enemy_iter != enemiesArray.end(); enemy_iter++) {
            if (bulletsArray[counter]->rect.getGlobalBounds().intersects(
                    enemiesArray[counter1]->rect.getGlobalBounds()) &&
                bulletsArray[counter]->isPlayerBullet) {
                if (bulletsArray[counter]->isPlayerBullet &&
                    !enemiesArray[counter1]->isAgro) {
                    enemiesArray[counter1]->isAgro = true;
                    enemiesArray[counter1]->movementSpeed *= 1.5;
                }
                CollisionText *text = new CollisionText(
                        "-" + std::to_string(bulletsArray[counter]->damage),
                        "pixel_font", 24, 6000);
                text->text.setPosition(
                        enemiesArray[counter1]->rect.getPosition().x,
    ...
    for (enemy_iter = enemiesArray.begin();
            ...
            1000) // update every 1 sec
        {
            if (player->rect.getGlobalBounds().intersects(
                ...
                CollisionText *text = new CollisionText(
                        "-" + std::to_string(enemiesArray[counter]->damage),
                        "pixel_font", 30, 6000);
                text->text.setPosition(player->rect.getPosition().x,
                                       player->rect.getPosition().y);
                text->color = sf::Color(255, 36, 0, 255);
                textsArray.push_back(text);
                player->hp -= enemiesArray[counter]->damage;

                CollisionText *text1 = new CollisionText(
                        "-" + std::to_string(player->damage),
                        "pixel_font", 24, 6000);
                text1->text.setPosition(
                        enemiesArray[counter]->rect.getPosition().x,
    ...
    for (item_iter = itemsArray.begin();
         item_iter != itemsArray.end(); item_iter++) {
        if (player->rect.getGlobalBounds().intersects(
                itemsArray[counter]->rect.getGlobalBounds()) &&
...
void Game::backpackState() {
    ...
    backpack->setPosition(player->rect.getPosition().x,
                          player->rect.getPosition().y);
    backpack->update(player);
    if (sf::Keyboard::isKeyPressed(sf::Keyboard::Num4) &&
        player->haveSecondWeapon) {
        Weapon *weapon = player->throwSecondWeapon();
        weapon->rect.setPosition(player->rect.getPosition().x,
                                 player->rect.getPosition().y);
        weapon->update();
        weapon->isExist = true;
        weaponArray.push_back(weapon);
    ...
    if (timer.asMilliseconds() >= 500) {
        if (sf::Keyboard::isKeyPressed(sf::Keyboard::Num2) &&
            player->haveSecondWeapon) {
            if (player->coins >= player->secondWeapon->upgateCost &&
                player->secondWeapon->level < 5) {
    ...
    if (timer.asMilliseconds() >= 500) {
        if (sf::Keyboard::isKeyPressed(sf::Keyboard::Num1)) {
            if (player->coins >= player->currentWeapon->upgateCost &&
                player->currentWeapon->level < 5) {
...
void Game::pauseState() {
    ...
    pause_text.setPosition(
            player->rect.getPosition().x - window->getSize().x / 16,
            player->rect.getPosition().y - window->getSize().y / 16);
    window->draw(pause_rect);
