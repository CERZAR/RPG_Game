...
void Game::initWindow() {
  ...
  // sf::Style::Fullscreen);
  this->window =
      new sf::RenderWindow(sf::VideoMode(1360, 768), "Game", sf::Style::Close);
...
void Game::initBackpackState() { backpack = new Backpack(resContainter); }
...
Game::~Game() {
  counter = 0;
  for (bullet_iter = bulletsArray.begin(); bullet_iter != bulletsArray.end();
       bullet_iter++) {
  ...
  counter = 0;
  for (text_iter = textsArray.begin(); text_iter != textsArray.end();
       text_iter++) {
  ...
  counter = 0;
  for (item_iter = itemsArray.begin(); item_iter != itemsArray.end();
       item_iter++) {
  ...
  counter = 0;
  for (weapon_iter = weaponArray.begin(); weapon_iter != weaponArray.end();
       weapon_iter++) {
...
void Game::updateGUI() {
  ...
  if (sf::Keyboard::isKeyPressed(sf::Keyboard::Tab)) {
    ...
    lvl_text.setPosition(player->rect.getPosition().x -
                             window->getSize().x / 2 + window->getSize().x / 36,
                         player->rect.getPosition().y -
                             window->getSize().y / 2 + 81);
    exp_text.setString("Exp: " + std::to_string(player->exp) + "/" +
                       std::to_string(player->needExp));
    exp_text.setPosition(player->rect.getPosition().x -
                             window->getSize().x / 2 + window->getSize().x / 36,
                         player->rect.getPosition().y -
                             window->getSize().y / 2 + 101);
  }
}
    ...
    for (enemy_iter = enemiesArray.begin(); enemy_iter != enemiesArray.end();
      ...
      if (bulletsArray[counter]->rect.getGlobalBounds().intersects(
        ...
        CollisionText *text = new CollisionText(
            "-" + std::to_string(bulletsArray[counter]->damage), "pixel_font",
            24, 6000);
  ...
  for (enemy_iter = enemiesArray.begin(); enemy_iter != enemiesArray.end();
    ...
    if (player_enemy_collision_timer.asMilliseconds() >=
      ...
      if (player->rect.getGlobalBounds().intersects(
        ...
        CollisionText *text = new CollisionText(
            "-" + std::to_string(enemiesArray[counter]->damage), "pixel_font",
            30, 6000);
        ...
        CollisionText *text1 = new CollisionText(
            "-" + std::to_string(player->damage), "pixel_font", 24, 6000);
...
void Game::backpackState() {
  ...
  pause_rect.setPosition(player->rect.getPosition().x - window->getSize().x / 2,
                         player->rect.getPosition().y -
                             window->getSize().y / 2);
...
void Game::update() {
  ...
  player_enemy_collision_timer =
      player_enemy_collision_clocker.getElapsedTime();
  timer = clocker.getElapsedTime();
