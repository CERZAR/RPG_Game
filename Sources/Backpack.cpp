...
void Backpack::update(Player* player)
{
	...
	backpackShapeArray[6]->setOutlineColor(player->currentWeapon->rect.getOutlineColor());
	...
	if (player->haveSecondWeapon == true)
	{
		...
		backpackShapeArray[8]->setOutlineColor(player->secondWeapon->rect.getOutlineColor());
	...
	backpackTextArray[2]->setString("HP: " + std::to_string(player->hp) + "/" + std::to_string(player->maxHP));
	backpackTextArray[3]->setPosition(x + 152, y + 120);
	backpackTextArray[3]->setString("MP: " + std::to_string(player->mp) + "/" + std::to_string(player->maxMP));
	backpackTextArray[4]->setPosition(x + 152, y + 145);
	backpackTextArray[4]->setString("EXP: " + std::to_string(player->exp) + "/" + std::to_string(player->needExp));
	...
	if (player->haveSecondWeapon == true)
	{
		...
		backpackTextArray[12]->setString("LVL: " + std::to_string(player->currentWeapon->level));
		backpackTextArray[13]->setPosition(x + 820, y + 340);
		backpackTextArray[13]->setString("DAMAGE: " + std::to_string(player->currentWeapon->damage));
		backpackTextArray[14]->setPosition(x + 820, y + 365);
		backpackTextArray[14]->setString("ATACK SPEED: " + std::to_string(player->currentWeapon->atackSpeed));
		backpackTextArray[15]->setPosition(x + 820, y + 390);
		backpackTextArray[15]->setString("BULLET SPEED: " + std::to_string((int)player->currentWeapon->speed));
		backpackTextArray[16]->setPosition(x + 820, y + 415);
		backpackTextArray[16]->setString("FLIGHT TIME: " + std::to_string((int)player->currentWeapon->lifetime / 1000));
		...
		if (player->currentWeapon->level == 5)
		{
			...
			backpackTextArray[17]->setString("UPGRATE COST: " + std::to_string(player->currentWeapon->upgateCost));
	}

	backpackTextArray[18]->setPosition(x + 820, y + 65);
	backpackTextArray[18]->setString("LVL: " + std::to_string(player->currentWeapon->level));
	backpackTextArray[19]->setPosition(x + 820, y + 90);
	backpackTextArray[19]->setString("DAMAGE: " + std::to_string(player->currentWeapon->damage));
	backpackTextArray[20]->setPosition(x + 820, y + 115);
	backpackTextArray[20]->setString("ATACK SPEED: " + std::to_string(player->currentWeapon->atackSpeed));
	backpackTextArray[21]->setPosition(x + 820, y + 140);
	backpackTextArray[21]->setString("BULLET SPEED: " + std::to_string((int)player->currentWeapon->speed));
	backpackTextArray[22]->setPosition(x + 820, y + 165);
	backpackTextArray[22]->setString("FLIGHT TIME: " + std::to_string((int)player->currentWeapon->lifetime / 1000));
	...
	if (player->currentWeapon->level == 5)
	{
		...
		backpackTextArray[23]->setString("UPGRATE COST: " + std::to_string(player->currentWeapon->upgateCost));
}

void Backpack::render(sf::RenderTarget * target)
{
	counter = 0;
	for (backpackShape_iter = backpackShapeArray.begin(); backpackShape_iter != backpackShapeArray.end(); backpackShape_iter++)
	{
	...
	counter = 0;
	for (backpackText_iter = backpackTextArray.begin(); backpackText_iter != backpackTextArray.end(); backpackText_iter++)
	{
	...
	if (player->haveSecondWeapon == true) target->draw(*iconSecondWeapon);
}
